...
    async update(id: number, data: IUpdateFlashSale): Promise<{ message: string, flashSale: FlashSaleEntity }> {
        // Hàm update của repository (TypeORM) sẽ tự động merge các trường trong data,
        // miễn là IUpdateFlashSale (từ DTO) đã bao gồm quantity.
...
            }
            ...
            // Logic kiểm tra số lượng tồn kho đã có sẵn ở đây
