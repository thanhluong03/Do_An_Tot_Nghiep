...
export class ClassificationAttributeRelationshipRepository {
    ...
    async findByProductId(productId: number): Promise<ClassificationAttributeRelationshipEntity[]> {
        return await this.repository
            ...
            .where('class1.product_id = :productId OR class2.product_id = :productId', { productId })
    ...
    async update(id: number, data: Partial<ClassificationAttributeRelationshipEntity>): Promise<void> {
        await this.repository.update({ id }, data);
    }

    async findById(id: number): Promise<ClassificationAttributeRelationshipEntity | null> {
