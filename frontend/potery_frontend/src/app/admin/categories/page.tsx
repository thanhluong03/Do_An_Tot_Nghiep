// src/app/admin/category/CategoryPage.tsx
...
export default function CategoryPage() {
    ...
    // Logic Phân trang cơ bản (tương tự StorePage)
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editingId) {
                // Sửa
                // Loại bỏ id, created_at, updated_at trước khi gửi
                const { id, created_at, updated_at, ...updateDto } = form;
                await updateCategory(editingId, updateDto);
                setEditingId(null);
                alert(`Cập nhật danh mục ID ${editingId} thành công!`);
            } else {
                // Thêm mới (gửi 1 item trong array)
    ...
    const handleEdit = (category: Category) => {
        // Sao chép toàn bộ object, đảm bảo ID được giữ lại
        setForm({ ...category });
    ...
    const handleDelete = async (id: number) => {
        if (!window.confirm(`Bạn có chắc muốn xoá danh mục ID ${id} không?`)) return;
        try {
            await deleteCategory(id);
            alert(`Xoá danh mục ID ${id} thành công!`);
            fetchCategories();
        } catch (error) {
            ...
            <div className="w-full mx-auto bg-white rounded-2xl shadow-lg p-6">
                ...
                <div className={`border p-6 rounded-lg mb-8 ${editingId ? 'border-yellow-400' : 'border-blue-400'}`}>
                    <h3 className="text-xl font-semibold mb-4 text-gray-700">
                         {editingId ? `Sửa Danh mục ID: ${editingId}` : "Thêm Danh mục mới"}
                    </h3>
                ...
                <div className="flex items-center justify-between mt-4">
                    <p className="text-sm text-gray-600">
                        Hiển thị {startIndex + 1} - {Math.min(startIndex + pageSize, categories.length)} trên {categories.length} danh mục
                    </p>
