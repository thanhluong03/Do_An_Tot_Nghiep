...
export default function AdminOrderPage() {
    ...
    // 1. Debounce logic & Reset trang khi tìm kiếm
    useEffect(() => {
    ...
    // 2. Reset trang khi đổi trạng thái/ngày
    useEffect(() => {
        setPagination(prev => ({ ...prev, page: 1 }));
    }, [startDate, endDate, orderStatusFilter, paymentStatusFilter]);

    // 3. Logic lấy thông tin Admin
    useEffect(() => {
    ...
    // 4. Tải danh sách khách hàng và tài xế
    useEffect(() => {
        getCustomers({})
            .then(setCustomers)
    ...
    async function fetchAllOrders() {
        ...
        try {
            ...
            const ordersWithNames = data.map((order) => {
                const customer = customers.find((u) => u.id === order.customer_id);
                return {
                    ...order,
                    customer_name: customer?.full_name || `Khách #${order.customer_id}`,
                    customer_email: customer?.email, // Lấy luôn email khách hàng
    ...
    // 6. HÀM LỌC VÀ PHÂN TRANG LOCAL
    const fetchOrders = useMemo(() => {
        if (allOrders.length === 0 && !loading) {
            setTotalOrders(0);
            setOrders([]);
            setKeyAndDateFilteredOrders([]);
            return;
        }
        ...
        if (endDate) {
            ...
            end.setDate(end.getDate() + 1); // Lọc đến hết ngày cuối
        ...
        // Danh sách này sẽ được dùng để tính toán số liệu cho các tab (OrderStatusTabs).
        ...
        startDate,
        endDate,
        loading // Thêm loading để đảm bảo không chạy khi allOrders chưa sẵn sàng
    ]);


    // 7. useEffect để tải ALL ORDERS khi Customers và StoreID thay đổi
    useEffect(() => {
    ...
    // 8. useEffect để LỌC VÀ PHÂN TRANG LOCAL khi ALL ORDERS hoặc bộ lọc thay đổi
    useEffect(() => {
        ...
        startDate,
        endDate,
        fetchOrders // Đảm bảo gọi lại logic lọc khi dependencies thay đổi
    ]);
    ...
    // 9. Cập nhật: handleDeleteConfirmed (Không gọi fetchAllOrders())
    const handleDeleteConfirmed = async () => {
        ...
        try {
            ...
            // ✅ Cập nhật allOrders bằng cách lọc bỏ đơn hàng vừa xóa
    ...
    // 10. Cập nhật: handleOrderUpdated (Không gọi fetchAllOrders() và tìm order trong allOrders)
    const handleOrderUpdated = async (orderId: number, updateData: any) => {
        ...
        try {
            ...
            if (customerEmail && order?.is_login_customer === false) {
                ...
                else if (updateData.status === "CANCELLED") await sendOrderRejectedMail({ orderId, to: customerEmail }); // Giả định CANCELLED là trạng thái từ chối
            }
    ...
    // 11. Cập nhật: handleAssignDriver (Vẫn cần fetchAllOrders() vì việc gán driver thường liên quan đến data trên server)
