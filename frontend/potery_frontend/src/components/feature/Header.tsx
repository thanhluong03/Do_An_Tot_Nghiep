...
// import { CartCountContext } from '../../contexts/CartContext'; // 4. ĐÃ XÓA (vì không dùng)

// 2. ĐÃ DI CHUYỂN 'declare global' ra ngoài (Top-level)
declare global {
  interface Window {
...
export const Header: React.FC = () => {
  // 3. ĐÃ XÓA 'declare global' khỏi đây

  // 4. ĐÃ XÓA/COMMENT BIẾN KHÔNG DÙNG
  ...
  const isActiveHref = (href: string) => {
    ...
    if (query) {
      try {
        ...
        return false;
      } catch { // 5. ĐÃ SỬA: Xóa '(e)' không dùng
        return false;
      }
    }
