// src/components/adminUsers/UserTable.tsx
"use client";
import React from "react";
// ✅ Import getUserAvatarUrl cùng với User
            ...
            <table className="min-w-full bg-white text-sm">
                <thead className="bg-indigo-50 border-b border-indigo-200 text-left font-semibold text-indigo-800">
                    <tr>
                        ...
                        <th className="px-4 py-3">ID Vai trò</th>
                ...
                <tbody>
                    {users.length > 0 ? (
                        users.map((u) => {
                            // ✅ Sử dụng hàm trợ giúp để lấy URL/Data URI ảnh
                            const avatarSrc = getUserAvatarUrl(u);
                            
                            // Kiểm tra nếu avatarSrc là URL mặc định để hiện placeholder
                            const hasAvatar = avatarSrc !== "/no-image.jpg";

                            return (
                                <tr key={u.id} className="border-t border-gray-100 hover:bg-indigo-50 transition duration-100">
                                    ...
                                    <td className="px-4 py-3 font-medium text-indigo-600">{u.role_id}</td>
